---
? after testing and analysis, most of the transactions packaged in the block are also
  in the pending queue, and the transactions in the pending queue are successfully
  verified. so whether it can be considered, if the transaction in the block also
  exists in the pending queue, it is not necessary to verify the signature. of course,
  there is a special case that needs to be considered. in the scenario of a `accountupdatecontract`
  transaction, the `accountupdatecontract` transaction will change the account permissions,
  so the final logic can be implemented as follows
: ''
