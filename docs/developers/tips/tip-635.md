---
'* phase1': Since TIP-53 and before TIP-465 took effect, the voting reward withdrawal
  is calculated accumulatively, and the time complexity is O(n).
'* phase2': After TIP-465, its voting reward withdrawal adopts a more efficient calculation
  method, and the time complexity is O(1).
