...
- Final
- Standards Track
the ecdsa signature scheme in [rfc6919](https: //tools.ietf.org/html/rfc6979) with
  elliptic curve [secp256k1](http://www.secg.org/sec2-v2.pdf) returns the signature
  `(r, s, v)`, where `r` and `s` are the values
tip: '120'
