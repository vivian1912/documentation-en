---
adds a new opcode at 0xf5, which takes 4 stack arguments: endowment, memory_start,
  memory_length, salt. Behaves identically to CREATE, except using `addressPrefix
  ++ keccak256(address ++ salt ++ keccak256(init_code))[12:]` to create a new contract
  address. `addressPrefix` is `0x41` forT
hide:
- toc
tags:
- Unknown
- Unknown
? the create2 has the same `engery` schema as `create`, but also an extra `hashcost`
  of `gsha3word * ceil(len(init_code) / 32)`, to account for the hashing that must
  be performed. the `hashcost` is deducted at the same time as memory-expansion engery
  and `creat`e is deducted
: before evaluation of the resulting address and the execution of ``.
---
