---
? at present, most public chains in the industry use embedded databases as their underlying
  storage, and tron is no exception. currently, tron supports leveldb and rocksdb
  two kinds of kv storage, and leveldb is the default configuration, which is also
  the storage solution adopted by most nodes. however, there is a problem when using
  leveldb as storage at present
: when the machine goes down abnormally, there is a certain chance that the database
  will be inconsistent. This article will describe the causes of database corruption
