...
- Final
- Standards Track
the shielded trc-20 contract has three core modules: '`mint`, `transfer` and `burn`.
  `mint` is used to transform the public TRC-20 token to shielded token, which makes
  token ownership invisible. `transfer` is used for shielded token transactions, which
  can hide the source address, the destination address, and the transaction amount.
  `burn` is used to transform the shielded token to the public TRC-20 token. The technical
  implementation is based on zk-SNARK proof system, which is secure and efficient.'
tip: '135'
